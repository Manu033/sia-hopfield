...
async function trainDefault() {
    try {
        ...
        status.textContent = '❌ Error al cargar letras';
    }
}
...
async function addToStore() {
    try {
        ...
        status.textContent = `✓ Patrón "${lab}" guardado y W actualizada`;
    } catch (e) {
        status.textContent = '❌ Error al guardar patrón';
    }
}
...
async function fetchW() {
    try {
        ...
        wDiv.innerHTML = '<p class="muted">Error cargando matriz W</p>';
    }
}
...
async function refreshLearned() {
    try {
        ...
        if (labels.length === 0) {
            learnedDiv.innerHTML = '<p class="muted">Sin patrones entrenados</p>';
            return;
        }
        ...
        learnedDiv.innerHTML = '<p class="muted">Error cargando patrones</p>';
    }
}
...
async function recognize() {
    try {
        ...
        const data = await api('/api/recognize', 'POST', { vector: vec, max_steps: 20 });
        ...
        data.steps.forEach((st, idx) => {
            ...
            h3.textContent = `Paso ${k}`; 
            card.appendChild(h3);
            
            const r1 = document.createElement('div'); r1.className = 'row';
            const hlabel = document.createElement('span'); hlabel.className = 'kbd'; hlabel.textContent = 'h = W·x';
            const htext = document.createElement('span'); htext.className = 'muted'; 
            htext.textContent = ` [${h.join(', ')}]`;
            ...
            const stext = document.createElement('span'); stext.className = 'muted'; 
            stext.textContent = ` [${s.join(', ')}]`;
        ...
        const matchStatus = data.match ? `✓ ${data.match}` : '✗ No coincide';
        txt.innerHTML = `
            <div><b>Resultado Final</b></div>
            <div style="word-break: break-all; font-size: 12px; max-width: 500px;">
                Vector: <span class="kbd">[${data.final.join(', ')}]</span>
            </div>
            <div>Match exacto: <span class="kbd">${matchStatus}</span></div>
            <div>Más cercano (Hamming): <span class="kbd">${data.nearest_label ?? '—'}</span> (dist = ${data.nearest_hamming ?? '—'})</div>
            <div>Energía: <span class="kbd">${Number(data.energy_final).toFixed(2)}</span></div>
          `;
        finalRow.appendChild(txt);
        diag.appendChild(finalRow);
    } catch (e) {
        status.textContent = '❌ Error en reconocimiento';
    }
}
